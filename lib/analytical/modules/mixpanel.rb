  ...
  module Modules
    class Mixpanel
      ...
      def track(event, properties = {})
        ...
        %(if (!window['disableAnalytical']) { mixpanel.track("#{event}", #{properties.to_json}, #{callback}); })
      end

      # Used to set "Super Properties" - http://mixpanel.com/api/docs/guides/super-properties
      def set(properties)
        "if (!window['disableAnalytical']) { mixpanel.register(#{properties.to_json}); }"
      end
      ...
      def identify(id, *args)
        ...
        %(if (!window['disableAnalytical']) { mixpanel.identify('#{id}');#{name_str} })
      end

      def event(name, attributes = {})
        %(if (!window['disableAnalytical']) { mixpanel.track("#{name}", #{attributes.to_json}); })
      end
